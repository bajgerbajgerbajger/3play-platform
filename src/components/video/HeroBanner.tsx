...
export function HeroBanner({ items }: HeroBannerProps) {
  ...
  const handleLoadedMetadata = async () => {
    ...
    const isMovie = !!item && !('seasons' in item);

    if (videoRef.current && item && isMovie && (!(item as Movie).duration || (item as Movie).duration === 0) && !durations[item.id] && isAdmin) {
  ...
  useEffect(() => {
    ...
    const totalMinutes = (item && 'duration' in item ? (item as Movie).duration : null) || (item ? durations[item.id] : null) || 120;
    ...
    const sceneTimer = setInterval(() => {
      if (showVideo) {
        ...
        randomizeScene(item && 'duration' in item ? (item as Movie).duration : undefined);
      }
        ...
        <div className="max-w-3xl space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-4 pt-4 animate-in fade-in slide-in-from-top duration-1000 delay-300">
            <Link
              ...
              className={cn(
                buttonVariants({ size: 'lg', className: 'bg-red-600 hover:bg-red-700 text-white font-black uppercase tracking-widest rounded-none h-14 px-8 shadow-[0_0_20px_rgba(220,38,38,0.3)] transition-all hover:scale-105' })
              )}
            >
